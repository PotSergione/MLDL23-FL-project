...
# Added by our group to build the Rotated Femnist centralized experiment
class RotateTransform(object):
  """
  Randomly rotate an image of degrees in a given array
  """
  ...
  def __call__(self, img):
    """
    Args:
        img (numpy array): Image to be randomly rotated.
    Returns:
        numpy array: Randomly rotated image.
    """

    # Randomly select a rotation degree from the list
    degree = torch.randint(low=0, high=len(self.degrees), size=(1,)).item()

    # Apply the selected rotation to the image
    return transforms.functional.rotate(img, self.degrees[degree], fill=1.0)


# Added by our group to rotate the decentralized femnist dataset
class Rotate(object): 
  """
  Rotate an image of degrees in a given array
  """
  def __init__(self, degrees):
      self.degrees = degrees
  ...
  def __call__(self, img):
    ...
    return transforms.functional.rotate(img, self.degrees, fill=1.0)
