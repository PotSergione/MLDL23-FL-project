...
def load_data(name):
    ...
    for f in files:
        ...
        for j in data['users']:
            classes_per_user.append(np.unique(data['user_data'][j]['y']).shape[0])

    return users, num_samples, classes_per_user

def print_dataset_stats(name):
    users, num_samples, classes_per_user = load_data(name)
    num_users = len(users)
    ...
    print('mean classes seen by user: %.2f'  % np.mean(classes_per_user))# found to be 54.988
    print('standard dev of classes seen by user: %.2f' % np.std(classes_per_user))
    
    bins = [0, 20, 40, 60, 80, 100, 120, 140, 160, 180, 200]
    if args.name == 'shakespeare':
        bins = [0, 2000, 4000, 6000, 8000, 10000, 12000, 14000, 16000, 18000, 20000]
    if args.name == 'femnist':
        bins = [2 * i for i in range(25)]
    if args.name == 'celeba':
